...
protected[mcrl2] class FreshNames[A](val description: String,
  ...
  val unstore = scala.collection.mutable.LinkedHashMap[String, A]()
  ...
  def unapply(x:String): String = synchronized {
    if (unstore.contains(x)) (unstore(x).toString())
    else throw new RuntimeException(s"Unexpected unapply in Spec: ${x}") 
  }
...
class Spec(global: Option[mpstk.GlobalType], ctx: mpstk.Context,
  ...
  def getMPSTName(x: String): String = {
    if (x.startsWith("r(")) roles.unapply(x)
    else if (x.startsWith("s(")) sessions.unapply(x)
    else if (x.startsWith("m(")) labels.unapply(x)
    else throw new RuntimeException(s"Unexpected mCRL2 name for backwards translation: ${x}")
  }
